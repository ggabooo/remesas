...
def layout(**kw):
    ...
    base = dict(
        ...
        title_font=dict(size=15, color=GOLD),
...
def pg_comparacion(df):
    ...
    with c1:
        ...
        fig.update_yaxes(title_text="Remesas (MUSD)",        secondary_y=False, showgrid=True,  gridcolor=f"{GOLD}22")
...
def pg_espuria(z):
    ...
    with c1:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=df_r.index.to_timestamp(how="end"), y=res,
        ...
        fig.update_layout(**layout(
            title=f"Residuales en el tiempo  (DW = {dw:.3f})", height=330))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with c2:
        ...
        fig2.add_hline(y=-ci, line_dash="dash", line_color=RED, line_width=1)
        fig2.update_layout(**layout(
            title="ACF de residuales - hay memoria?", height=330, bargap=0.15))
        st.plotly_chart(fig2, use_container_width=True)

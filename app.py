...
def pg_inicio(df):
    ...
    fig.update_layout(**ql(
        ...
        xaxis=dict(title="Trimestre"), yaxis=dict(title="MUSD"), height=380
    ))
...
def pg_comparacion(df):
    ...
    with c2:
        ...
        fig2.update_layout(**ql(
            ...
            yaxis=dict(title="Remesas GT (MUSD)"), height=360
        ))
        st.plotly_chart(fig2, use_container_width=True)
...
def pg_coint(z, ur_tbl):
    ...
    fig.update_layout(**ql(title="Coeficientes del modelo · Verde = significativo (p<0.05)",
                           xaxis=dict(title="Valor del coeficiente"), height=360))
    st.plotly_chart(fig, use_container_width=True)
